function outputFolder = align_pairdir(inputPath, outputFolder, varargin)
% ALIGN_PAIRDIR Align a subdir of sequences with CTW.
% ALIGN_PAIRDIR(DIRECTORY, OUTPUTFOLDER, FEATURES) writes the
% alignment vectors of a subdir. Subdir must consist of
% paired sequences. I.e. In a folder of {seq1, seq2,
% seq3,...} seq1 and seq2 are pairs, seq3 and seq4 are pairs
% and so on.
%
% Input:
...
fprintf(1,'align_pairdir: Checking arguments...\n');
if isempty(varargin),
    features = [];
elseif (length(varargin)==1),
    features = varargin{1};
else
    error('align_pairdir:argChk','Wrong number of input arguments!');
end
...
for i = 1:2:length(data)-1,
    for j = i+1,
        ...
        try
            ...
            % could be somehow more helpfull...
